...
export default function Emblem({ size = 52, tone = 'default', linkHome = false, still = false, className = '' }) {
  ...
  // `still` strips the animation classes so a rasterized snapshot (the QR poster
  // export) captures a stable, fully-settled frame instead of a mid-animation one.
  const anim = (cls) => (still ? undefined : cls)
  ...
  const svg = (
    <svg viewBox="0 0 90 78" className="w-full h-auto block" aria-hidden="true">
      ...
      <g fill={palette.flower} opacity="0.9">
        <circle className={anim('eo-flower')} cx="31" cy="26" r="3" />
        <circle className={anim('eo-flower')} cx="37" cy="21" r="2.3" />
        <circle className={anim('eo-flower')} cx="59" cy="26" r="3" />
        <circle className={anim('eo-flower')} cx="53" cy="21" r="2.3" />
      </g>
      ...
      {/* drifting rose petals — subtle on tiny logos, lovely on the big one.
          Hidden when still: a frozen mid-fall petal reads as a stray dot. */}
      {!still && (
        <g fill="#c2a0b0">
      ...
      <g fill="#c2a25c">
        <circle className={anim('eo-sparkle')} cx="45" cy="13" r="1.1" style={{ animationDelay: '0.3s' }} />
        <circle className={anim('eo-sparkle')} cx="35" cy="20" r="0.9" style={{ animationDelay: '1.4s' }} />
        <circle className={anim('eo-sparkle')} cx="56" cy="20" r="0.9" style={{ animationDelay: '2.1s' }} />
      </g>
    </svg>
  )
